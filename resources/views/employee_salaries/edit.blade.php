...
@endif
    ...
    <div class="card-body">
        ...
        <form id="expense-entry" action="{{ route('employee_salaries.update',$emp_salary->salaryId) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
            ...
            <div class="form-group">
                ...
                <select name="emp_Id" id="emp_Id" class="form-control @error('emp_Id') is-invalid @enderror">
                    ...
                    @foreach($emp as $em)
                    <option value="{{$em->emp_Id}}" {{old("emp_Id",$emp_salary->emp_Id)==$em->emp_Id?"selected":""}}>{{$em->emp_fname .' '.$em->emp_lname .'('. $em->pos_name.')'}}</option>
                    @endforeach
                </select>
                @error('emp_Id')
            ...
            <div class="form-group">
                ...
                <input type="text" name="monthly_salary" class="form-control @error('monthly_salary') is-invalid @enderror" id="monthly_salary"
                    placeholder="Employee Salary Amount" value="{{ old('monthly_salary',$emp_salary->monthly_salary) }}">
            ...
            <div class="form-group">
                ...
                <select name="salary_status" class="form-control @error('salary_status') is-invalid @enderror" id="salary_status">
                    <option value="1" {{ old('salary_status',$emp_salary->salary_status) == 1 ? 'selected' : ''}}>Active</option>
                    <option value="0" {{ old('salary_status',$emp_salary->salary_status) == 0 ? 'selected' : ''}}>Inactive</option>
                </select>
                @error('salary_status')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
            ...
            <button class="btn btn-lg btn-primary float-right text-white" type="submit">Update Salary</button>
        </form>
</div>
</div>
</div>
