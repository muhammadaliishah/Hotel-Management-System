            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">Edit Employee Payroll</li>
                    </ol>
                </div>
            </div>
        </div><!-- /.container-fluid -->
...
@endif
    ...
    <div class="card-body">
        <form id="expense-entry" action="{{ route('employees_payroll.update',$payroll->payroll_Id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        	<table class="table table-striped table-bordered" style="width:50%;">
        		...
        		<tbody>
        			<tr>
        				<td>
                            <input type="hidden" name="loanId" value="{{$payroll->loanId}}">
        					<select name="emp_Id" id="emp_Id" class="form-control @error('emp_Id') is-invalid @enderror">
			                    <option value="">--Select Employee--</option>
			                    @foreach($emp as $em)
			                    <option value="{{$em->emp_Id}}" {{$payroll->emp_Id==$em->emp_Id?"selected":""}}>{{$em->emp_fname.' '.$em->emp_lname.'('. $em->pos_name.')'}}</option>
			                    @endforeach
                			</select>
			                @error('emp_Id')
                    ...
                    <option value="1" {{ $payroll->salary_month === 1 ? 'selected' : ''}}>January</option>
                    <option value="2" {{ $payroll->salary_month === 2 ? 'selected' : ''}}>February</option>
                    <option value="3" {{ $payroll->salary_month === 3 ? 'selected' : ''}}>March</option>
                    <option value="4" {{ $payroll->salary_month === 4 ? 'selected' : ''}}>April</option>
                    <option value="5" {{ $payroll->salary_month === 5 ? 'selected' : ''}}>May</option>
                    <option value="6" {{ $payroll->salary_month === 6 ? 'selected' : ''}}>June</option>
                    <option value="7" {{ $payroll->salary_month === 7 ? 'selected' : ''}}>July</option>
                    <option value="8" {{ $payroll->salary_month === 8 ? 'selected' : ''}}>August</option>
                    <option value="9" {{ $payroll->salary_month === 9 ? 'selected' : ''}}>September</option>
                    <option value="10" {{ $payroll->salary_month === 10 ? 'selected' : ''}}>October</option>
                    <option value="11" {{ $payroll->salary_month === 11 ? 'selected' : ''}}>November</option>
                    <option value="12" {{ $payroll->salary_month === 12 ? 'selected' : ''}}>December</option>
                </select>
                @error('salary_month')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                ...
                @enderror
    					...
    					<td>
    						<select name="salary_year" class="form-control @error('salary_year') is-invalid @enderror" id="salary_year">
                    			<option value="{{$currentYear}}" {{ $payroll->salary_year ===  $currentYear? 'selected' : ''}}>{{$currentYear}}</option>
			                </select>
			                @error('salary_year')
			                <span class="invalid-feedback" role="alert">
			                    <strong>{{ $message }}</strong>
        	...
        	<table class="table table-bordered table-sm">
        		...
        		<tbody>
        			<tr style="background: #0000000D;">
        				...
        				<td>
        					<input type="text" name="days_worked" class="form-control @error('days_worked') is-invalid @enderror" id="days_worked"
                   				 placeholder="Number of days worked" value="{{ old('days_worked',$payroll->days_worked) }}">
			                @error('days_worked')
			                <span class="invalid-feedback" role="alert">
			                    <strong>{{ $message }}</strong>
			                </span>
        				...
        				<td><input type="text" name="mn_salary" id="mn_salary" class="form-control input-number" value="{{ old('mn_salary') }}" readonly></td>
        			</tr>
        			<tr style="background: #0000000D;">
        				<td>2</td>
                    ...
                    placeholder="Number of overtime hours worked" value="{{ old('overtime_hrs_worked',$payroll->overtime_hrs_worked) }}">
                ...
                @enderror</td>
                    ...
                    placeholder="Overtime hours fare amount(Rs)" value="{{ old('overtime_hrs_fare',$payroll->overtime_hrs_fare) }}">
                ...
                @enderror</td>
                    ...
                    placeholder="Number of hours late" value="{{ old('hours_late',$payroll->hours_late) }}">
                @error('hours_late')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
                @enderror</td>
                    ...
                    placeholder="Late hours amount deduction(Rs)" value="{{ old('hours_late_deduction',$payroll->hours_late_deduction) }}">
                ...
                @enderror</td>
                    ...
                    placeholder="Number of days absent" value="{{ old('days_absent',$payroll->days_absent) }}">
                @error('days_absent')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
                @enderror</td>
                    ...
                    placeholder="Absent days amount deduction" value="{{ old('days_absent_deduction',$payroll->days_absent_deduction) }}">
                ...
                @enderror</td>
                    ...
                    placeholder="Bonus description" >{{ old('bonus_description',$payroll->bonus_description) }}</textarea>
                ...
                @enderror</td>
                    ...
                    placeholder="Bonus amount (if any)" value="{{ old('bonus_amount',$payroll->bonus_amount) }}">
                @error('bonus_amount')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
                @enderror</td>
                    ...
                    placeholder="Loan deduction amount" value="{{ old('loan_deduction',$payroll->loan_deduction) }}">
                ...
                @enderror</td>
                    ...
                    placeholder="Estimated monthly salary" value="{{ old('estimated_month_salary',$payroll->estimated_month_salary) }}" readonly>
            ...
            <button class="btn btn-lg btn-primary float-right text-white" type="submit">Update Monthly Salary</button>
        </form>
</div>
</div>
</div>
