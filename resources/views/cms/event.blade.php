...
@endif
    ...
    <div class="float-right">
            ...
            <a class="btn btn-danger"  href="{{ route('event.create')}}"><i class="fa fa-plus text-white"></i></a>
    </div>
    </div>
            ...
            <table class="table">
                ...
                @foreach ($data as $key=>$Vendor)
                    <tr>
                        ...
                        <td><img width="40px" height="40px" src="/{{$Vendor->image}}" alt=""></td>
                        <td>{{$Vendor->date}}</td>
                        <td>{{$Vendor->time}}</td>
                        <td>
                            <a href="{{ route('event.edit', $Vendor->id) }}">
                            <button class="btn btn-info">Update</button>
                        </a>

                        <form action="{{ route('event.destroy', $Vendor->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
