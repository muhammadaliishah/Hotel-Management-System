...
@endif
    ...
    <div class="card-body">
        ...
        <form id="expense-entry" action="{{ route('employee_positions.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
            ...
            <div class="form-group">
                ...
                <select name="pos_type" class="form-control @error('pos_type') is-invalid @enderror" id="pos_type">
                    <option value="Regular" {{ old('pos_type') === 'Regular' ? 'selected' : ''}}>Regular</option>
                    <option value="Contractual" {{ old('pos_type') === 'Contractual' ? 'selected' : ''}}>Contractual</option>
                </select>
                @error('pos_type')
            ...
            <div class="form-group">
                ...
                <input type="text" name="pos_name" class="form-control @error('pos_name') is-invalid @enderror" id="pos_name"
                    placeholder="Position Name" value="{{ old('pos_name') }}">
                @error('pos_name')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
            ...
            <div class="form-group">
                ...
                <select name="pos_status" class="form-control @error('pos_status') is-invalid @enderror" id="pos_status">
                    <option value="1" {{ old('pos_status') == 1 ? 'selected' : ''}}>Active</option>
                    <option value="0" {{ old('pos_status') == 0 ? 'selected' : ''}}>Inactive</option>
                </select>
                @error('pos_status')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
            ...
            <button class="btn btn-lg btn-primary float-right" type="submit">Create Position</button>
        </form>
</div>
</div>
</div>
